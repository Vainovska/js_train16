...
function logArrayElements(arr) {
  ...
  arr.forEach((value, index) => console.log(`Елемент ${index}: ${value}`));
...
function compareArrays(arr1, arr2) {
  ...
  const a = JSON.stringify(arr1.sort());
  const b = JSON.stringify(arr2.sort());
  console.assert(a === b, "Масиви не однакові!");
...
function traceBackward(n) {
  ...
  return n === 1 ? console.trace("myCode") : traceBackward(n - 1);
...
function validateUserInput(userInput) {
  ...
  console.assert(userInput.password.length !== 0, "Помилка: пароль відсутній!");
